...
describe('Session Cookie Encryption', () => {
  ...
  describe('parseSessionCookie', () => {
    ...
    it('should return null for wrong secret (tampered)', async () => {
      const cookie = await createSessionCookie('sess_123', 'user_456', TEST_SECRET);
      const parsed = await parseSessionCookie(cookie, 'wrong-secret-key-also-32bytes!!!');
  ...
  describe('Legacy cookie backward compatibility', () => {
    it('should parse legacy format cookies (sessionId:userId:signature)', async () => {
      // Create a legacy-style cookie manually
      // This simulates cookies created before the encryption update
      const legacyPayload = 'sess_legacy:user_legacy';

      // We need to create a valid HMAC signature for the legacy format
      // Since the signing is internal, we'll test that valid legacy cookies work
      // by creating one using the internal functions indirectly

      // For now, verify the format detection works
...
describe('Session Cookie Header', () => {
  describe('createSessionCookieHeader', () => {
    it('should create a proper Set-Cookie header', async () => {
      const header = await createSessionCookieHeader('sess_123', 'user_456', TEST_SECRET);
    ...
    it('should respect custom max age', async () => {
      ...
      const header = await createSessionCookieHeader('sess_123', 'user_456', TEST_SECRET, customMaxAge);
...
describe('Device Fingerprinting', () => {
  describe('getDeviceId', () => {
    ...
    it('should include Sec-CH-UA headers in fingerprint', async () => {
      ...
      const deviceIdWith = await getDeviceId(requestWithClientHints, TEST_SECRET);
      const deviceIdWithout = await getDeviceId(requestWithoutClientHints, TEST_SECRET);
  ...
  describe('parseDeviceName', () => {
    it('should identify iPhone', () => {
      expect(parseDeviceName('Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X)')).toBe('iPhone');
    });

    it('should identify iPad', () => {
      expect(parseDeviceName('Mozilla/5.0 (iPad; CPU OS 16_0 like Mac OS X)')).toBe('iPad');
    });

    it('should identify Android Phone', () => {
      expect(parseDeviceName('Mozilla/5.0 (Linux; Android 13; Pixel 7) Mobile')).toBe('Android Phone');
    });

    it('should identify Android Tablet', () => {
      expect(parseDeviceName('Mozilla/5.0 (Linux; Android 13; SM-T970)')).toBe('Android Tablet');
    });

    it('should identify Chrome on Mac', () => {
      expect(parseDeviceName('Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) Chrome/120')).toBe('Chrome on Mac');
    });

    it('should identify Safari on Mac', () => {
      expect(parseDeviceName('Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) Safari/605')).toBe('Safari on Mac');
    });

    it('should identify Firefox on Mac', () => {
      expect(parseDeviceName('Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:120.0) Firefox/120')).toBe('Firefox on Mac');
    });

    it('should identify Edge on Windows', () => {
      expect(parseDeviceName('Mozilla/5.0 (Windows NT 10.0; Win64; x64) Edg/120')).toBe('Edge on Windows');
    });

    it('should identify Chrome on Windows', () => {
      expect(parseDeviceName('Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120')).toBe('Chrome on Windows');
    });

    it('should identify Chrome on Linux', () => {
      expect(parseDeviceName('Mozilla/5.0 (X11; Linux x86_64) Chrome/120')).toBe('Chrome on Linux');
    });

    it('should identify Chromebook', () => {
      expect(parseDeviceName('Mozilla/5.0 (X11; CrOS x86_64) Chrome/120')).toBe('Chromebook');
    });
...
describe('Security: Cookie Tamper Detection', () => {
  it('should reject cookies with modified sessionId', async () => {
    const cookie = await createSessionCookie('original_session', 'user_123', TEST_SECRET);

    // Try to create a cookie with different sessionId
    const maliciousCookie = await createSessionCookie('stolen_session', 'user_123', TEST_SECRET);
    ...
    const maliciousParsed = await parseSessionCookie(maliciousCookie, TEST_SECRET);
  ...
  it('should reject cookies with modified userId', async () => {
    const cookie = await createSessionCookie('sess_123', 'legitimate_user', TEST_SECRET);
  ...
  it('should use different derived keys for different secrets', async () => {
    const cookie = await createSessionCookie('sess_123', 'user_456', 'secret_key_one_32bytes!!!!!!!!!');

    // Cookie encrypted with one secret can't be decrypted with another
    const parsedWrongKey = await parseSessionCookie(cookie, 'secret_key_two_32bytes!!!!!!!!!');
