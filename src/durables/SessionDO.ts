...
export class SessionDO extends DurableObject<SessionDOEnv> {
  ...
  async validateSession(
    ...
    const result = await this.ctx.storage.sql
      .exec(`SELECT * FROM sessions WHERE id = ? AND expires_at > ?`, sessionId, now)
      .toArray();
  ...
  async revokeSession(sessionId: string): Promise<boolean> {
    ...
    const result = await this.ctx.storage.sql.exec(`DELETE FROM sessions WHERE id = ?`, sessionId);
  ...
  async listSessions(): Promise<Session[]> {
    ...
    const rows = await this.ctx.storage.sql
      .exec(`SELECT * FROM sessions WHERE expires_at > ? ORDER BY last_active_at DESC`, now)
      .toArray();
  ...
  async recordLoginAttempt(success: boolean): Promise<void> {
    ...
    if (success) {
      ...
      await this.ctx.storage.sql.exec(`DELETE FROM rate_limits WHERE key = 'login'`);
    } else {
