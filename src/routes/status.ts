...
status.post('/incidents', async (c) => {
	...
	const { title, type, impact, components, initialStatus, initialMessage } = body;

	if (!title || !type || !impact || !components?.length || !initialStatus || !initialMessage) {
...
status.post('/incidents/:id/updates', async (c) => {
	...
	const update = await statusQueries.addIncidentUpdate(db, id, newStatus, message);
