...
describe('POST /auth/device-code', () => {
  ...
  async function makeDeviceCodeRequest(body: Record<string, string>) {
    const app = createApp();
    return app.request('/auth/device-code', {
      method: 'POST',
  ...
  describe('code generation', () => {
    ...
    it('returns verification URIs', async () => {
      ...
      expect(json.verification_uri_complete).toContain(`${mockEnv.AUTH_BASE_URL}/auth/device?user_code=`);
    });
    ...
    it('accepts JSON request body', async () => {
      const res = await makeDeviceCodeJsonRequest({ client_id: 'grove-cli', scope: 'openid email' });
      expect(res.status).toBe(200);
      const json = await res.json() as DeviceCodeResponse;
      expect(json.device_code).toBeDefined();
    });
...
describe('POST /auth/device/authorize', () => {
  ...
  async function makeAuthorizeRequest(body: Record<string, string>, asJson = false) {
    const app = createApp();
    if (asJson) {
      return app.request('/auth/device/authorize', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      }, mockEnv, mockExecutionCtx);
  ...
  describe('approve action', () => {
    ...
    it('redirects to success page (form submission)', async () => {
      const res = await makeAuthorizeRequest({ user_code: 'ABCD-1234', action: 'approve' });
      expect(res.status).toBe(302);
      const location = res.headers.get('Location');
  ...
  describe('deny action', () => {
    ...
    it('denies device code', async () => {
      ...
      expect(res.status).toBe(200);
      expect(denyDeviceCode).toHaveBeenCalledWith(
        expect.anything(),
        'dc-1'
      );
    });
    ...
    it('redirects to denied page (form submission)', async () => {
      const res = await makeAuthorizeRequest({ user_code: 'ABCD-1234', action: 'deny' });
      expect(res.status).toBe(302);
      const location = res.headers.get('Location');
