...
export function createApiClient(request: APIRequestContext) {
  return {
     ...
     */
    async verifyToken(token: string): Promise<{ active: boolean; sub?: string; email?: string }> {
     ...
     */
    async getUserInfo(token: string): Promise<{ sub: string; email: string; name: string | null }> {
