 ...
 */
async function verifyBearerToken(c: { req: { header: (name: string) => string | undefined }; env: Env }) {
...
subscription.post('/:userId/post-count', async (c) => {
  ...
  if (body.action === 'increment') {
    ...
    return c.json({
      ...
      error_description: 'Body must contain either { action: "increment" | "decrement" } or { count: number }'
    }, 400);
  }
