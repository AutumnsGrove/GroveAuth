...
vi.mock('../middleware/rateLimit.js', () => ({
  passkeyRegisterRateLimiter: vi.fn((_c: unknown, next: () => Promise<void>) => next()),
  passkeyDeleteRateLimiter: vi.fn((_c: unknown, next: () => Promise<void>) => next()),
  passkeyAuthRateLimiter: vi.fn((_c: unknown, next: () => Promise<void>) => next()),
}));
...
describe('Better Auth Handler - passkey security', () => {
  ...
  it('applies stricter CSP to passkey routes', async () => {
    const app = createApp();
    const res = await app.request('/api/auth/passkey/register', { method: 'GET' }, mockEnv);
  ...
  it('rate limits passkey registration', async () => {
    const app = createApp();
    await app.request('/api/auth/passkey/verify-registration', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    }, mockEnv);
  ...
  it('rate limits passkey deletion', async () => {
    const app = createApp();
    await app.request('/api/auth/passkey/delete-passkey', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    }, mockEnv);
  ...
  it('rate limits passkey authentication', async () => {
    const app = createApp();
    await app.request('/api/auth/passkey/verify-authentication', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    }, mockEnv);
...
describe('Better Auth Handler - audit logging', () => {
  ...
  it('logs passkey_registered on successful registration', async () => {
    const mockHandler = mockAuthHandler(new Response(JSON.stringify({ success: true }), { status: 200 }));
    (createAuth as ReturnType<typeof vi.fn>).mockReturnValue({ handler: mockHandler });
  ...
  it('logs passkey_auth_failed on failed auth', async () => {
    const mockHandler = mockAuthHandler(new Response(JSON.stringify({ error: 'failed' }), { status: 400 }));
    (createAuth as ReturnType<typeof vi.fn>).mockReturnValue({ handler: mockHandler });
    (createAuditLog as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
