...
minecraft.delete('/mods/:filename', async (c) => {
  ...
  return proxyToMcControl(c, 'DELETE', `/api/mc/mods/${encodeURIComponent(filename)}`);
});
...
minecraft.post('/backups/:id/restore', async (c) => {
  ...
  const response = await fetch(`${MC_CONTROL_URL}/api/mc/backups/${backupId}/restore`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
...
minecraft.get('/backups/:id/download', async (c) => {
  ...
  const response = await fetch(`${MC_CONTROL_URL}/api/mc/backups/${backupId}/download`, {
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${token}`,
    },
  ...
  return new Response(response.body, {
    headers: {
      ...
      'Content-Disposition': response.headers.get('Content-Disposition') || `attachment; filename="${backupId}.tar.gz"`,
    },
  });
});
