...
test.describe('Passkey Management', () => {
  ...
  test('should display empty state when no passkeys registered', async ({ page }) => {
    await page.goto('/dashboard/security');

    // Should show empty state or "no passkeys" message
    await expect(
      page.getByText(/no passkeys/i).or(page.getByText(/add your first passkey/i))
  ...
  test('should list all registered passkeys', async ({ page }) => {
    ...
    await expect(
      page.getByText(/passkey registered/i).or(page.getByText(/passkey added/i))
    ).toBeVisible({ timeout: 15000 });

    // Verify the list shows the passkey
    const passkeyList = page.locator('[data-testid="passkey-list"]').or(page.locator('.passkey-list'));
  ...
  test('should allow renaming a passkey', async ({ page }) => {
    ...
    if (await editButton.isVisible()) {
      ...
      await expect(page.getByText('My MacBook Pro Passkey')).toBeVisible({ timeout: 5000 });
    } else {
  ...
  test('should allow deleting a passkey with confirmation', async ({ page, webauthn }) => {
    // Register a passkey first
    await page.goto('/dashboard/security');
  ...
  test('should show passkey metadata (created date, last used)', async ({ page }) => {
    // Register a passkey
    await page.goto('/dashboard/security');
  ...
  test('should prevent deleting last passkey if only auth method', async ({ page }) => {
