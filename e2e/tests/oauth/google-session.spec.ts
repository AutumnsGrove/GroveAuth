...
test.describe('OAuth Session Replay', () => {
  ...
  test.describe('with saved session', () => {
    ...
    test('should access dashboard without login prompt', async ({ page }) => {
      ...
      await expect(
        page.getByRole('heading', { name: /dashboard/i }).or(page.getByText(/welcome/i))
    ...
    test('should show user info in header', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="user-avatar"]').or(page.locator('.user-menu')).or(page.getByRole('img', { name: /avatar/i }))
      ).toBeVisible({ timeout: 10000 });
    });
