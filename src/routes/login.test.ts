...
describe('GET /login - page rendering', () => {
  ...
  it('includes Discord sign-in link', async () => {
    const res = await makeLoginRequest();
    const html = await res.text();
    expect(html).toContain('/api/auth/sign-in/social?provider=discord');
  });
...
describe('GET /login - returnTo parameter', () => {
  ...
  it('preserves returnTo in magic link form', async () => {
    const returnTo = 'https://auth.grove.place/auth/device?user_code=ABCD-1234';
    const res = await makeLoginRequest(`?returnTo=${encodeURIComponent(returnTo)}`);
    const html = await res.text();
...
describe('GET /login - legacy OAuth params', () => {
  ...
  it('handles missing state gracefully', async () => {
    ...
    const res = await makeLoginRequest(`?redirect_uri=${encodeURIComponent(redirectUri)}`);
    expect(res.status).toBe(200);
    const html = await res.text();
...
describe('GET /login - provider redirect', () => {
  ...
  it('redirects to Discord when provider=discord', async () => {
    const res = await makeLoginRequest('?provider=discord');
    expect(res.status).toBe(302);
