...
test.describe('Session Management', () => {
  test('should return null session when not authenticated', async ({ request }) => {
  ...
  test('should preserve return URL after login redirect', async ({ page }) => {
    ...
    const url = page.url();
    const hasReturnUrl =
      url.includes('redirect') || url.includes('returnTo') || url.includes('callbackUrl');

    // Log for debugging if return URL handling is different
    if (!hasReturnUrl) {
      console.log('Note: Return URL may be stored in session state instead of query param');
    }
  });
...
test.describe('Cross-Subdomain Sessions', () => {
  ...
  test('should set cookie with correct domain for production', async ({ page }) => {
