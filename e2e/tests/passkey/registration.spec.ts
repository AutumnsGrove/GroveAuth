...
test.describe('Passkey Registration', () => {
  ...
  test('should show passkey option on security settings page', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: /passkey/i }).or(page.getByText(/passkey/i))
    ).toBeVisible({ timeout: 10000 });
  });

  test('should register a new passkey successfully', async ({ page, webauthn }) => {
  ...
  test('should handle registration cancellation gracefully', async ({ page, webauthn }) => {
  ...
  test('should fail registration when user verification fails', async ({ page, webauthn }) => {
  ...
  test('should show registered passkey in list after creation', async ({ page }) => {
    await page.goto('/dashboard/security');
    ...
    await expect(
      page.getByText(/my passkey/i).or(page.locator('[data-testid="passkey-item"]'))
