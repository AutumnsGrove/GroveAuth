...
import { createAccessToken, verifyAccessToken, decodeToken, isTokenExpired, getTokenTimeRemaining } from './jwt.js';
...
describe('verifyAccessToken - rejections', () => {
  ...
  it('rejects token signed with wrong key', async () => {
    ...
    const { privateKey: otherPrivateKey } = await jose.generateKeyPair(JWT_ALGORITHM);
...
describe('isTokenExpired', () => {
  ...
  it('returns true at exact expiry time', () => {
    const expTime = Math.floor(new Date('2025-06-01T01:00:00Z').getTime() / 1000);
