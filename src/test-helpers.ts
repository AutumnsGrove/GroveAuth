/**
 * Shared test infrastructure for GroveAuth tests
 */
 ...
 */
export function createSequentialMockDb(responses: Array<{
  first?: unknown;
  all?: { results: unknown[] };
  run?: { success: boolean };
}>) {
  ...
  return {
    prepare: vi.fn().mockImplementation(() => {
      const response = responses[callIndex] ?? { first: null, all: { results: [] }, run: { success: true } };
      callIndex++;
      return {
        bind: vi.fn().mockReturnValue({
 ...
 */
export function createFormRequest(url: string, params: Record<string, string>, headers: Record<string, string> = {}): Request {
 ...
 */
export function createJsonRequest(url: string, body: unknown, headers: Record<string, string> = {}): Request {
  return new Request(url, {
    method: 'POST',
    headers: {
