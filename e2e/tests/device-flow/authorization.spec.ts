...
test.describe('Device Code Authorization Flow', () => {
  test('should initiate device code flow and return codes', async ({ request }) => {
  ...
  test('should show device authorization page with user code', async ({ page, request }) => {
    const api = createApiClient(request);

    // CLI initiates device code
    const { user_code, verification_uri } = await api.initiateDeviceCode(TEST_CLIENT.id);
    ...
    await expect(
      page.getByRole('textbox', { name: /code/i }).or(page.getByPlaceholder(/code/i))
    ).toBeVisible({ timeout: 10000 });

    // Enter the user code
    const codeInput = page.getByRole('textbox', { name: /code/i }).or(page.getByPlaceholder(/code/i));
    ...
    await expect(
      page.getByText(/authorize/i).or(page.getByText(/allow/i)).or(page.getByText(/approve/i))
    ).toBeVisible({ timeout: 10000 });
  });

  test('should pre-fill user code from verification_uri_complete', async ({ page, request }) => {
    const api = createApiClient(request);

    // CLI initiates device code
    const { verification_uri_complete } = await api.initiateDeviceCode(TEST_CLIENT.id);
  ...
  test('should complete full CLI authentication journey', async ({ page, request }) => {
    const api = createApiClient(request);
    ...
    await expect(
      page.getByText(/approved/i).or(page.getByText(/authorized/i)).or(page.getByText(/success/i))
    ).toBeVisible({ timeout: 10000 });
  ...
  test('should handle user denial', async ({ page, request }) => {
    ...
    await expect(
      page.getByText(/denied/i).or(page.getByText(/rejected/i)).or(page.getByText(/cancelled/i))
    ).toBeVisible({ timeout: 10000 });
  ...
  test('should return slow_down when polling too fast', async ({ request }) => {
    ...
    const hasSlowDown = results.some((r) => 'error' in r && r.error === 'slow_down');
    const allPending = results.every((r) => 'error' in r && r.error === 'authorization_pending');
  ...
  test('should handle expired device code', async ({ request }) => {
    ...
    const result = await api.pollDeviceCode('invalid-device-code', TEST_CLIENT.id);

    // Should return an error (expired_token or invalid request)
    expect('error' in result).toBe(true);
  });

  test('should show correct client name on authorization page', async ({ page, request }) => {
    const api = createApiClient(request);
    ...
    await expect(
      page.getByText(/grove cli/i).or(page.getByText(TEST_CLIENT.name, { exact: false }))
    ).toBeVisible({ timeout: 10000 });
  });
});
